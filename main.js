$(function(){
    ...
    const globalState = {
      ...
      dateTime: {
        meses: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'Octomber', 'November', 'December'],
        dias: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturady', 'Sunday']
      },
      bateriaBaja: false,
      draggScreen: false
    }
  ...
  <div class="modo">Slow motion</div>
  <div class="modo">Video</div>
  <div class="modo active">Photo</div>
  <div class="modo">Portrait</div>
  <div class="modo">Panorama</div>
  </div>
  ...
  <path d="M62 38l-7.259-9.86L46 36"></path>
    ...
    function renderizarUI(){
      ...
      if ($('.wrapperApps .app[data-app="widgetFullCalendario"]').length) {
        ...
        $('.wrapperApps .app[data-app="widgetFullCalendario"] .icono').append('<div class="eventos"><p>No more events for today</p></div><div class="calendarioWrapper"></div>');
  ...
  <div class="menuOption eliminar">delete app
  <div class="icono">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  ...
  `);
    ...
    $('body').on('click', '.fixedMenuFixedApp .menuOption.eliminar', function () {
      ...
      if (idApp == undefined) {
        var idDeck = $('#fixedApp').data('deck');
      }
      $(this).parent().remove();
      $('#fixedApp').remove();
      $('.mainScreen').removeClass('filterBlur');
      alertaiOS({
        encabezado: `To transfer ${idApp !== undefined ? globalState.apps[idApp].nombre : 'app'} to the app library or delete the app?`,
        ...
        acciones: [
          ...
          },
          {
            ...
            callback: function () { console.log('Pending app library') }
          },
          {
            texto: 'Cancelar'
          },
    ...
    $('.appScreen').on('click', '.app .removeApp', function () {
      ...
      if (idApp == 'undefined') {
        var idDeck = $(this).parent('.app').data('deck');
      }
      $('.appScreen .app .removeApp').remove();
      $('.mainScreen').removeClass('shakingApps');
      alertaiOS({
